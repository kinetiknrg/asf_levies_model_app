...
with st.sidebar:
    ...
    if st.session_state.approach == "Create my own":
        ...
        st.write(f"🔍 DEBUG: Processing approach '{st.session_state.approach}'")

        # Create a deep copy of the cached levies to prevent mutation errors on re-runs
        st.write("🔍 DEBUG: Creating levies copy...")
        levies_copy = copy.deepcopy(levies)

        st.write("🔍 DEBUG: Getting approach weights...")
        try:
            st.session_state.rebalancing_weights = get_approach_weights(
                levies_copy, st.session_state.approach
            )
            st.write("✅ DEBUG: Weights retrieved successfully")
        except Exception as e:
            st.error(f"❌ WEIGHTS FAILED: {type(e).__name__}: {str(e)}")
            st.code(traceback.format_exc())
            st.stop()


# Rebalance levies based on chosen approach - DEBUG VERSION
st.write("🔍 DEBUG: About to rebalance levies...")
st.write(f"DEBUG: Approach = {st.session_state.approach}")
st.write(f"DEBUG: Weights available = {bool(st.session_state.rebalancing_weights)}")

if not st.session_state.rebalancing_weights:
    st.error("❌ CRITICAL: Rebalancing weights are empty!")
    st.stop()
...
try:
    ...
    st.write("✅ DEBUG: Rebalancing successful")
except Exception as e:
    st.error(f"❌ REBALANCING FAILED: {type(e).__name__}")
...
# Validate app rates against official Ofgem data
